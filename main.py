...
class OCRProcessor:
    ...
    def _process_pdf(self, file_path: Path) -> str:
        images = pdf2image.convert_from_path(str(file_path), dpi=300, poppler_path="/run/current-system/sw/bin")  # đường dẫn poppler trên Linux/NixOS
        texts = [self._ocr_image(image) for image in images]
        return "\n\n--- Page Break ---\n\n".join(texts)
    ...
    def process_file(self, file_path: Union[str, Path]) -> str:
        ...
        if self._is_pdf(path):
            ...
            raise ValueError(
                f"Unsupported file format: {path.suffix}. " f"Supported formats: {', '.join(self.SUPPORTED_IMAGE_FORMATS | {self.SUPPORTED_PDF_FORMAT})}"
            )
